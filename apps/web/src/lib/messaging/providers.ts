...
/** Docker env files sometimes keep quotes or wrap a long key onto two lines. AskEva decrypts this key, and a broken length surfaces as OpenSSL "wrong final block length". */
export function normalizeAskEvaToken(raw: string) {
...
export function isUsableAskEvaToken(token: string) {
  return /^[a-fA-F0-9]+$/.test(token) && token.length >= 32 && token.length % 32 === 0;
}

export function askEvaTokenCandidates() {
  const seen = new Set<string>();
  const cleaned = [env("ASKEVA_API_TOKEN"), env("WHATSAPP_ACCESS_TOKEN")]
    .map(normalizeAskEvaToken)
    .filter((token) => {
      if (!token || seen.has(token)) return false;
      seen.add(token);
      return true;
    });
  const usable = cleaned.filter(isUsableAskEvaToken);
  return usable.length > 0 ? usable : cleaned;
}

function askevaToken() {
  return askEvaTokenCandidates()[0] ?? "";
}
...
async function postWhatsAppTemplate(params: {
  ...
  const tokens = askEvaTokenCandidates();
  if (tokens.length === 0) {
    return { ok: false, error: "AskEva API token is missing." };
  }

  let last: SendResult = { ok: false, error: "AskEva API token is missing." };
...
async function postAskEvaTemplate(
  token: string,
  params: {
    to: string;
    name: string;
  ...
  try {
    ...
    console.error("[whatsapp] AskEva request failed", error instanceof Error ? error.message.slice(0, 300) : error);
...
function whatsappNetworkError(error: unknown) {
  ...
  const message = `${error instanceof Error ? error.message : "WhatsApp request failed."} ${cause}`.trim();
